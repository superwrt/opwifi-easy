...
class OwStatags extends Model
{
    ...
    {
        return $this->belongsToMany('App\Models\OwStations', 'ow_statag_relationships', 'tag_id', 'sta_id');
    }
    ...
    static public function groups($nameAlias = null)
    {
		...
		$groups = DB::table('ow_stagroups')->get();
		$tags = OwStatags::get()->toArray();
    ...
    static public function modifyGroups($r, $nameAlias = null)
    {
		...
		$oldGroups = DB::table('ow_stagroups')->get();
		$oldTags = OwStatags::get()->toArray();
	    ...
	    if (!self::flat_item($groups, $tags, $newNum, $r))
	    	return false;
	    self::treeApply($groups, $oldGroups, $nameAlias, 'parent_id', 'ow_stagroups', $idMap);
	    self::treeApply($tags, $oldTags, $nameAlias, 'group_id', 'ow_statags', $idMap);
